...
#ifndef Pythia8_LHAMadgraphUser_H
#define Pythia8_LHAMadgraphUser_H
...
class LHAupMadgraphUser : public LHAup {
...
public:
  ...
  // Constructor.
  LHAupMadgraphUser(Pythia* pythiaIn, bool matchIn = true,
...
LHAupMadgraphUser::LHAupMadgraphUser(Pythia *pythiaIn, bool matchIn, string dirIn,
                             string exeIn) :
...
LHAupMadgraphUser::~LHAupMadgraphUser() {if (lhef) delete lhef; if (hook) delete hook;}
...
// readString("", LHAupMadgraphUser::Configure).

bool LHAupMadgraphUser::readString(string line, Stage stage) {
  if (stage == Auto) {
...
void LHAupMadgraphUser::addCard(string src, string dst) {
...
bool LHAupMadgraphUser::setSeed(int seedIn, int runsIn) {
  ...
  if (seed < 0) {
    ...
    if (seed < 1) {
      pythia->info.errorMsg("Error from LHAupMadgraphUser::setSeed: the given "
                            "Pythia seed is less than 1."); return false;}
  }
  runs = runsIn;
  if (seed * runs > 30081 * 30081) {
    pythia->info.errorMsg("Error from LHAupMadgraphUser::setSeed: the given seed "
...
void LHAupMadgraphUser::setEvents(int eventsIn) {events = eventsIn;}
...
void LHAupMadgraphUser::setJets(int jetsIn) {jets = jetsIn;}

//--------------------------------------------------------------------------

// Execute a system command.

bool LHAupMadgraphUser::execute(string line) {return system(line.c_str()) != -1;}
...
bool LHAupMadgraphUser::configure() {
...
bool LHAupMadgraphUser::generate() {
  ...
  if (!success) {orig.close(); return false;}
    ...
    pythia->info.errorMsg("Error from LHAupMadgraphUser::generate: MadGraph "
...
bool LHAupMadgraphUser::launch() {
  ...
  if (amcatnlo) {
    if (access((dir + "/SubProcesses/results.dat").c_str(), F_OK) == -1) {
      pythia->info.errorMsg("Error from LHAupMadgraphUser::launch: aMC@NLO failed "
    ...
    if (access((dir + "/" + gpk).c_str(), F_OK) == -1) {
		pythia->info.errorMsg("Error from LHAupMadgraphUser::launch: MadEvent failed"
				" to produce " + gpk);
		return false;
	}
...
bool LHAupMadgraphUser::launchOnly() {
  ...
  if (amcatnlo) {
    string line = "cd " + dir + "/MCatNLO/lib; LINKS=`ls`; for LINK in $LINKS;"
      ...
      "rm $LINK; ln -s ${TARG:3} $LINK; fi; done";
    if (!execute(line)) {
      pythia->info.errorMsg("Error from LHAupMadgraphUser::launchOnly: failed to "
                            "link aMC@NLO libraries"); return false;}
  }
...
bool LHAupMadgraphUser::prepareRun(){
  if (amcatnlo) {
    if (access((dir + "/SubProcesses/results.dat").c_str(), F_OK) == -1) {
      pythia->info.errorMsg("Error from LHAupMadgraphUser::prepareRun: aMC@NLO failed "
                            "to produce results.dat"); return false;}
    fstream script((dir + "/run.sh").c_str(), ios::out);
    script << "#!/usr/bin/env bash\n"
           << "sed -i \"s/.*= *nevents/$1 = nevents/g\" ./Cards/run_card.dat\n"
    ...
    script.close(); execute("chmod 755 " + dir + "/run.sh");
	  ...
	  string gpk = "run_gridpack.tar.gz";
	  if (access((dir + "/" + gpk).c_str(), F_OK) == -1) {
		  pythia->info.errorMsg("Error from LHAupMadgraphUser::prepareRun: MadEvent failed"
				  " to produce " + gpk);
		  return false;
	  }
	  string line = "cd " + dir + "; tar -xzf " + gpk;
	  if (!execute(line)) {
		  pythia->info.errorMsg("Error from LHAupMadgraphUser::prepareRun: failed to "
				  "compile MadEvent code"); return false;}
  }
  return true;
}
...
bool LHAupMadgraphUser::run(int eventsIn, int seedIn) {

  if (!pythia) return false;
  if (nRuns >= runs) {
    pythia->info.errorMsg("Error from LHAupMadgraphUser::run: maximum number "
...
bool LHAupMadgraphUser::reader(bool init) {
  ...
  if (!lhef->setInit()) {
    pythia->info.errorMsg("Error from LHAupMadgraphUser::reader: failed to "
                          "initialize the LHEF reader"); return false;}
  if (lhef->sizeProc() != 1) {
    pythia->info.errorMsg("Error from LHAupMadgraphUser::reader: number of "
  ...
  if (init) {
    ...
    if (!amcatnlo) {
      ...
      if (vs.size() < 2) {
        pythia->info.errorMsg("Error from LHAupMadgraphUser::reader: could not "
...
bool LHAupMadgraphUser::setInit() {

  // Initialize MadGraph.
  if (!pythia) return false;
  if (access((dir + "/run.sh").c_str(), F_OK) == -1) {
    if (!configure()) {
      pythia->info.errorMsg("Error from LHAupMadgraphUser::setInit: failed to "
...
bool LHAupMadgraphUser::setEvent(int) {

  // Run setEvent from the LHEF object and launch MadGraph if failed.
  if (!pythia) return false;
  if (!lhef) {
    pythia->info.errorMsg("Error from LHAupMadgraphUser::setEvent: LHAupLHEF "
                          "object not correctly initialized"); return false;}
  if (!lhef->fileFound()) {
    pythia->info.errorMsg("Error from LHAupMadgraphUser::setEvent: LHEF "
