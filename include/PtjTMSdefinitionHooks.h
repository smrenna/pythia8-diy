...
double PtjTMSdefinitionHooks::tmsDefinition( const Event& event){
  ...
  if (partonSystemsPtr->sizeSys() > 1) return -1.0;

  // For shower veto, remember to keep track of reclusterings in counting.
  if (partonSystemsPtr->sizeSys() > 0) n_partons_in_born -= nRecluster();
...
ip=i;
  ...
  if (int(fjInputs.size()) == 0) {
    delete jetDef;
    //if (n_partons_in_born==0) return 1e10;
    return -1.0;
  }
...
//cout << endl;
  ...
  for (size_t i(0);i<jets.size();++i) {
    //Vec4 pj(jets[i].E(),jets[i].px(),jets[i].py(),jets[i].pz());
    Vec4 pj(jets[i].px(),jets[i].py(),jets[i].pz(),jets[i].E());


//cout << pj.pT() << " " << pj.rap() << endl;

    if (pj.pT()> ptmin && abs(pj.rap())< ymax) n++;
  }

//cout << n_partons_in_born << " " << jets.size() << " " << n << endl;
//
//if (event[ip].pT() > ptmin) {
//    delete jetDef;
////if (n==n_partons_in_born) { cout << event[ip].pT() << " " << event[ip].p().rap() << endl; abort();}
//    return 1e10;
//}
//
//if (event[ip].pT() < ptmin) {
//    delete jetDef;
//
////if (n_partons_in_born > 0 && n==n_partons_in_born) abort();
//
//    return 0.0;
//}

  // Veto events with too many hard jets.
  if (n > n_partons_in_born) { 
    delete jetDef;
    return 1e10;
  }
