...
bool SpaceShower::branch( Event& event) {
  ...
  int iA = partonSystemsPtr->getInA(iSysSel);
  int iB = partonSystemsPtr->getInB(iSysSel);
  vector<int> iOut;
  iOut.push_back(0);
  iOut.push_back(0);
  ...
  if ( (canVetoEmission
    ...
    if (canMergeFirst) {
      partonSystemsPtr->setInA(iSysSel, iA);
      partonSystemsPtr->setInB(iSysSel, iB);
      for (int iCopy = 2; iCopy < systemSizeOld; ++iCopy)
        partonSystemsPtr->setOut(iSysSel, iCopy - 2, iOut[iCopy]);
