...
void process_block(Block* b, diy::Master::ProxyWithLink const& cp,  bool verbose)
{
	...
	int seed = b->state.seed+cp.gid();
	b->pythia.readString("Random:setSeed = on");
	b->pythia.readString("Random:seed = " + std::to_string(seed));
	...
	if(b->state.use_mg5) {
		...
		// Now copy grippack files from GridPack generated by GenericBlock
		string dir_base(basepath(b->state.f_out)+"/GridPack");
		string dir(basepath(b->state.f_out)+"/run_"+std::to_string(cp.gid()));
		bool  amcatnlo =
			access((dir_base + "/Cards/amcatnlo_configuration.txt").c_str(), F_OK) != -1;
		...
		string line;
		if(amcatnlo) {
			// we have to copy the whole directory
			line = "cp -r "+dir_base+"/* "+dir+"/";
		}else{
	  		string gpk = "run_gridpack.tar.gz";
			line = "cp "+dir_base+"/" + gpk + " "+dir+"/";
		}
		if(system(line.c_str()) == -1){
			fmt::print(stderr, "{} cannot copy files\n", cp.gid());
			return ;
		}

		b->mg5->setEvents(b->state.num_events);
		b->mg5->setSeed(seed);
		b->mg5->prepareRun();
		...
		fmt::print(stderr, "{} MadGraph is Ready to Use\n", cp.gid());
	} else {
		// Configure pythia with a vector of strings
		for (auto s  : b->state.conf) {
			// fmt::print(stderr, "Pythis reads {}\n", s);
			b->pythia.readString(s);
		}
		...
		fmt::print(stderr, "[{}] Pythia read inputs\n", cp.gid());
	}
...
int main(int argc, char* argv[])
{
	...
	// master.log->set_level(::spdlog::level::debug);
	...
	//  Block all ranks
	fmt::print(stderr, "\n {} arrived and waiting", world.rank());
